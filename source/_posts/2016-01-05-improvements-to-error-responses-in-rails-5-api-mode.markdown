...
In previous posts, we showed how to build an API application with Rails 5. Specifically, we covered the integration with [Backbone](https://wyeworks.com/blog/2015/6/11/how-to-build-a-rails-5-api-only-and-backbone-application) and [Ember](https://wyeworks.com/blog/2015/6/30/how-to-build-a-rails-5-api-only-and-ember-application). At time of writing these posts, the only way to try out all the Rails API stuff was against the code on master, since Rails 5 was not yet released. However, [Rails-5-0-beta1 is now available](http://weblog.rubyonrails.org/2015/12/18/Rails-5-0-beta1/), so you can easily start to build things using Rails in this new API-flavored mode.

Today, I would like to comment about some changes and fixes related with the API mode that landed in Rails a few days ago. It's all about [improving the error responses for JSON requests on Rails](https://github.com/rails/rails/pull/20831). Our aim was to make sure that Rails is handling and returning responses in a proper format when something goes wrong processing a request.
...
In Rails, the concept of `format` appears just from the first scaffold command. For example, if you run a simple `bin/rails g scaffold post title`, you can find references to response formats in the controller and the routes.
...
In Rails controllers, you have the ability to write code that can respond in different formats. By default, generated code from the scaffold command can respond on HTML or JSON formats. That is easy to see in the `create` action, but it also happens in the `index` action where the render is implicit (this action returns a list of posts and this is formatted in the appropriate format).

Of course, there should be some sort of rule that determines which format is used to respond the request. Let's now take a look at the routes generated by the scaffold. The following is the output of `bin/rake routes`:
...
These routes are what you get when you have something like `resources: :post` in your routes file. We can see that `format` is a dynamic segment in all the routes and this is the main criteria in Rails to decide which is the format used to form a response.

So, an JSON response is generated if you issue a GET request to `http:://mysite.com/posts.json`and HTML response is sent if the URL is  `http:://mysite.com/posts.html`.  In the case of the `index` action, it just relies on the ability of Rails to render the list of posts in the format value taken from the route path. If format is not specified in the request path, or it's an unknown format, the response will be in HTML format (well, if your app is configured with `config.api_only = true`, hence it's using the API mode, the default format is JSON). 

By using `respond_to`, like in the 'create' action of our `PostController`, you can be specific about the content of the response depending on the specified format. In this case, if the format is not provided it will use the first format listed within the `respond_to` block. If there is not a handler for the specified format, it will raise a `ActionController::UnknownFormat` exception (and this will be mapped to a `406 Not Acceptable` HTTP error).
...
In the context of an API application, I personally prefer the use of the HTTP `Accept` header which is also supported in Rails, because this is the manner to specify the response format according to the HTTP protocol. The [HTTP/1.1 RFC](http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html) says:
